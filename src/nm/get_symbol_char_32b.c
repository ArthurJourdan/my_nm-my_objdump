...
static char get_symbol_char_from_sym(Elf32_Sym act_sym)
{
...
static char get_symbol_char_from_shdr(Elf32_Sym act_sym, Elf32_Shdr *act_sect)
{
...
char get_symbol_char_32(Elf32_Sym act_sym, Elf32_Shdr *act_sect)
{
    ...
    if (!my_sym_char)
        my_sym_char = '?';
    if (ELF32_ST_BIND(act_sym.st_info) == STB_LOCAL && my_sym_char != '?')
        my_sym_char += 32;
    return my_sym_char;
}